...
/obj/narsie/Initialize()
	...
	if(area)
		var/mutable_appearance/alert_overlay = mutable_appearance('icons/effects/bloodcult_effects.dmi', "ghostalertsie")
	...
	for (var/datum/antagonist/cult/bloodcult/cultist in GLOB.antagonists)
		if (!cultist.owner)
			continue
		all_cults |= cultist.cult_team

	for (var/_cult_team in all_cults)
		var/datum/team/cult/bloodcult/cult_team = _cult_team
	...
	for (var/datum/mind/cult_mind as anything in get_antag_minds(/datum/antagonist/cult/bloodcult))
...
/obj/narsie/proc/acquire(atom/food)
	...
	to_chat(old_target, span_bloodcult("NAR'SIE HAS LOST INTEREST IN YOU."))
	singularity_component.target = food
	if(ishuman(food))
		to_chat(food, span_bloodcult("NAR'SIE HUNGERS FOR YOUR SOUL."))
	else
		to_chat(food, span_bloodcult("NAR'SIE HAS CHOSEN YOU TO LEAD HER TO HER NEXT MEAL."))
