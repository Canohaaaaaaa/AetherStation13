...
/obj/ratvar
	...
	light_range = 15
	light_color = "#BE8700"
	var/clockwork_desc = "Ratvar, free at last!"
	var/atom/prey //Whatever Ratvar is chasing
	var/clashing = FALSE //If Ratvar is fighting with Nar'Sie
	var/convert_range = 10
	obj_flags = CAN_BE_HIT | DANGEROUS_POSSESSION

/obj/ratvar/Initialize()
	. = ..()
	START_PROCESSING(SSobj, src)
	send_to_playing_players(span_ratvar(("ONCE AGAIN MY LIGHT SHINES AMONG THESE PATHETIC STARS")))
...
/obj/ratvar/examine(mob/user)
	. = ..()
	if(IS_CULTIST_CLOCK(user))
		. += clockwork_desc

//ATTACK GHOST IGNORING PARENT RETURN VALUE
/obj/ratvar/attack_ghost(mob/dead/observer/O)
	var/alertresult = alert(O, "Embrace the Justiciar's light? You can no longer be revived!", "Transcend","Yes", "No")
...
/obj/ratvar/process()
	...
	for(var/mob/living/L in GLOB.alive_mob_list) //we want to know who's alive so we don't lose and retarget a single person
		if(L.z == z && !IS_CULTIST_CLOCK(L) && L.mind)
			meals += L
	...
	if(!prey)
		if(!prey && LAZYLEN(meals))
			...
			prey = pick(meals)
			to_chat(prey, span_clockcultbold("You will do, heretic."))
			to_chat(prey, span_userdanger("You feel something massive turn its crushing focus to you..."))
		...
		if((!istype(prey, /obj/narsie) && prob(10) && LAZYLEN(meals) > 1) || prey.z != z || !(prey in meals))
			...
			if(IS_CULTIST_CLOCK(mob_prey))
				to_chat(prey, span_clockcultbold("Serve me well.\n"))
				to_chat(prey, span_clockcult("You feel great joy as your god turns His eye to another heretic..."))
			else
				to_chat(prey, span_clockcultbold("No matter. I will find you later, heretic.\n"))
				to_chat(prey, span_userdanger("You feel tremendous relief as the crushing focus relents..."))
			prey = null
		else
...
/obj/ratvar/proc/clash_of_the_titans(obj/singularity/narsie/narsie)
